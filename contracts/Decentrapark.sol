...
contract Decentrapark {
    ...
    event ParkingAdded(address _owner, uint _rentPrice, uint _ParkingPrice); 
    ...
    function setRentPrice(uint256 _rentPrice) private returns(uint256) {
        //
    }

    function setParkingPrice(uint256 _rentPrice) private returns(uint256) {
        //
    }

    function setRentTime(uint256 _rentPrice) private returns(uint256) {
        //
    }
    ...
    function unRentParking(uint256 _indexParking) public payable {
        ...
        require(getRentTimeByIndex(_indexParking) <= block.timestamp, "You cannot ask to unRent yet, wait till the contract finishes");
